...
import numpy as np
from baseline_models import mse_ref
...
def main():
    ...
    with tf.Session() as sess:
        ...
        with tf.name_scope('data'):
            iter_train = TF2FLRD(ipaths_train, batchsize=FLAGS.batch_size, buffersize=730, parse=parse_func)
            iter_val = TF2FLRD(ipaths_val, batchsize=730, buffersize=730, parse=parse_func)
            iter_test = TF2FLRD(ipaths_test, batchsize=730, buffersize=730, parse=parse_func)
        sess.run(iter_train.initializer)
        sess.run(iter_val.initializer)
        sess.run(iter_test.initializer)

        model = mse_ref(sess=sess, FLAGS=FLAGS)
        #model.run(iter_data=iter_train, iter_val=iter_val)
        model.run(iter_data=iter_test, train=False, load=False)
