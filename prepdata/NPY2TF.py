...
'''
Return 2D list of data_macro, with the first axis year (according to the defined order), second axis type of data
'''
def loadNPY(yrs = [2000, 2001], IPATH = data_macro.NPY_DATA_DIRECTORY, loadlist=['X0', 'X3', 'X6', 'Y3', 'Y6']):
...
'''
def dimSelect(data, types = np.arange(data_macro.Nparamens), heights = np.arange(data_macro.Nheight),
 latitudes = np.arange(data_macro.Nlatitude), longitudes = np.arange(data_macro.Nlongitude), allflag = 0):
...
'''
def loadSelect(yrs = [2000, 2001], IPATH = data_macro.NPY_DATA_DIRECTORY, loadlist=['X0', 'X3', 'X6', 'Y3', 'Y6'],
    types = np.arange(data_macro.Nparamens), heights = np.arange(data_macro.Nheight),
     latitudes = np.arange(data_macro.Nlatitude), longitudes = np.arange(data_macro.Nlongitude)):
...
def quickNPY2TF(data, yrs, xinds, yinds, axis=1, comment=''):
    for i in range( len(yrs) ):
        opath = data_macro.TF_DATA_DIRECTORY + "/tf_" + str(yrs[i]) + comment
